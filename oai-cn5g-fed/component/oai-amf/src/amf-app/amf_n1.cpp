...
// RAB edit start Functions
std::string hex_2_string(const uint8_t* data, size_t size) {
    std::stringstream ss;
...
//RAB edit stop
  ...
  if (identity_response->ie_mobility_id) {
    ...
    // RAB edit start
    std::memcpy(random_r_ue, imsi.random_r, 8);
    //RAB edit stop
  }
  ...
  std::memcpy(nc->random_r, random_r_ue, 8);
  std::string random_r_Hex = hex_2_string(nc->random_r, 8);
  Logger::amf_n1().debug("Random R added to the NAS context: %s", random_r_Hex.c_str());
    ...
    if (auts) {
      ...
      for (int i = 0; i < MAX_5GS_AUTH_VECTORS; i++) {
        ...
        generate_5g_he_av_in_udm(
            ...
            vector[i], nc->random_r);  // serving network name
      }
...
void amf_n1::generate_5g_he_av_in_udm(
    ...
    std::string& serving_network, _5G_HE_AV_t& vector, const uint8_t random_r[8]) {
  ...
  Authentication_5gaka::f2345(
      ...
      ak,mk,rk,sk, random_r );  // to compute XRES, CK, IK, AK

  Authentication_5gaka::f1(
      opc, key, vector.rand, sqn, amf,
      mac_a, mk, random_r);  // to compute MAC, Figure 7, ts33.102
