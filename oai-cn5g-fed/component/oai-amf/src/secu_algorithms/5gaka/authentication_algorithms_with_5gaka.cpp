...
void Authentication_5gaka::f1(
    ...
    const uint8_t sqn[6], const uint8_t amf[2], uint8_t mac_a[8], uint8_t mk[8]  , const uint8_t random_r[8]) {
  uint8_t temp[16];
  uint8_t in1[16];
  uint8_t out1[16];
...
void Authentication_5gaka::f2345(
    const uint8_t opc[16], const uint8_t k[16], const uint8_t _rand[16],
    uint8_t res[8], uint8_t ck[16], uint8_t ik[16], uint8_t ak[6], uint8_t mk[8], uint8_t rk[8], uint8_t sk[8], const uint8_t random_r[8]) {
  uint8_t temp[16];
  uint8_t out[16];
  uint8_t rijndaelInput[16];
  uint8_t i;
...
// RAB remeber to delete the line below
  for (i = 0; i < 8; i++) res[i] = out[i + 8];
  // RAB here res is the res used to generate xres 
  ...
  for (i = 0; i < 16; i++) rijndaelInput[i] = random_r[i] ^ opc[i];
...
int Authentication_5gaka::generate_vector(
    ...
    uint8_t sqn[6], auc_vector_t* vector, const uint8_t random_r[8]) {
 ...
 /*
   ...
   */
  f2345(opc, key, vector->rand, vector->xres, ck, ik, ak, mk, rk, sk, random_r);
   ...
   * Compute MAC
   */
  f1(opc, key, vector->rand, sqn, amf, mac_a,mk,random_r);
