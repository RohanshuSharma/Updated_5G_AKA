...
std::string hex_to_string(const uint8_t* data, size_t size) {
    std::stringstream ss;
...
std::memcpy(supi_format_imsi->random_r, buf + decoded_size, 8);
decoded_size += 8;


std::string random_r_Hex = hex_to_string(supi_format_imsi->random_r, 8);
Logger::nas_mm().debug("Decoded Random R: %s", random_r_Hex.c_str());
